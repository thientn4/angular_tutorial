...
export class SpotifyService {
  ...
  searchFor(category:string, resource:string):Promise<ResourceData[]> {
    ...
    return this.sendRequestToExpress('/search/'+category+'/'+resource).then((data) => {
      return (
        category==='artist'?data.artists:
        (category==='album'?data.albums:
        data.tracks)
      ).items.map((item)=>{
        if(category==='artist')
  ...
  getAudioFeaturesForTrack(trackId:string):Promise<TrackFeature[]> {
    ...
    return this.sendRequestToExpress('/track-audio-features/'+trackId).then((data) => {
      console.log(JSON.stringify(data,null,2)) //thien
      let result:TrackFeature[]=[]
      for (var key in data){
        if (!["key","loudness","mode","tempo","type","id","uri","track_href","analysis_url","duration_ms","time_signature"].includes(key))
          result.push(new TrackFeature(key,Math.round(data[key]*10000)/100))
      }
      return result
    });
  }
